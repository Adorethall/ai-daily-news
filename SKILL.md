...
#### 方式二：飞书交互式卡片（推荐，原生飞书体验）
**在飞书群聊中使用时，必须输出为飞书原生交互式卡片 JSON 格式**，这是最佳体验。
...
```json
{
 ...
 "header": {
 "title": { "tag": "plain_text", "content": "📰 AI 日报 · {YYYY}年{MM}月{DD}日" },
 "template": "blue"
 },
 "elements": [
 {
...
格式规范：
- 使用 `interactive` 类型，启用 `wide_screen_mode`
- 头部标题 `📰 AI 日报 · YYYY年MM月DD日`，使用蓝色模板
- 分类标题加粗（`**🔥 头条**`）
- 每条新闻格式：`**【来源/公司】** 标题内容`
- 使用 `{"tag": "hr"}` 做分隔线
- 最底部注明数据来源
